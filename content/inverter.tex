...
Az áramköri szimulációk numerikus megoldása általában differenciálegyenlen rendserekre vezethetőek vissza. Az irodalom számos megoldásról említést tesz. A legtöbb esteben a modellezendő szakasz matematikai modelljét felállítjuk, egy ismert gerjesztésre való váalsz alapján, majd az így kepott modellt diszkretizáljuk.

Egy álatlános folytonos idejű rendszert az alábbi egenletek reprezentálnak: 
...
A legegyszerűbb numerikus megoldó módszer az Euler módszer, mely az állapotváltozók módosításán alapul, minden lépésben maguk az állapotváltozók és a bemenet aktuális értéke alapján. A diszkretizált egyenletrendszer az előre lépő Euler módszert alkalmazva az alábbi alakot ölti:
...
ahol $\underline{\underline{I}}$ az egységmátrix, T a lépésköz, n pedig az aktuális lépés indexe.
Egy másik változata a az előrelépő Euler módszernek a hátralépő Euler módszer, vagy más néven implicit Euler módszer. A különbség a két módszer között, hogy az implicit Euler az aktuális értékek helyett a következő lépés értékeit használja:
...
Mivel $\underline{x}[n+1]$ az egyenlet mindkét oldalán megjelenik, implicit módszerről beszélünk. Átrendezés után az állapotegyenelet az alábbi alakot öltik:
...
A formulából jól látszik, hogy az akutális érétk kiszámításához szükség van a változók ,,jövőbeni'' értékére. Erre következtethetünk a változók jelenlegi értékéből, vagy további késleltetést adva a rendszerhez összegyűjthetjük a számításhoz szükséges értékeket.
A két korábbi módszer kombinációja, mely széles körben alkalmazott szimulációs eljárások során a következő. Ez a módszer a trapéz módszer, a változók jelenlegi és következő lépésbeli értékk átlagát használja, így jobb közelítést adva, mintha két korábbi értéket használna:
...
Látható, hogy ez szintén egy implicit módszer, hiszen ez is tartalmazza $\underline{x}[n+1]$-et mindkét oldalon, illetve szintán támaszkodi ka gerjesztés jövőbeni értékére is. Kifejezve $\underline{x}[n+1]$-et az alábbi alakot kapjuk:
...
Az összes módszer mátrixműveletekre vezethető vissza, az implementációjukhoz csupán mátrix szorzásre és öszeadásra van szükség. Egy olyan szimulátor, amlegy általános állapotegyeneltek megoldására képes, igen ruglamas tud lenni, a mátrixok együtthatóinak változtatásásval, különböző rendszerek szimulációit hozhatjuk létre. További numerikus integráláson alapuló algoritmusok is állnak rendelkezésre, de ezek már másodrendű módszerek, amely azt jelenti, hogy szükségük van az állapotávltozók még korábbi értékeire, melyek igencsak megnöveli a számítási igényt.

Szintén gyakran használt közelítő algoritmus a negyedrendű Runge-Kutta módszer, mely a numerikus integrálástól eltérő megközelítést alkalmaz.
...
ahol $\underline{a}$, $\underline{b}$, $\underline{c}$, és $\underline{d}$ csak ideiglenesen használt vektorok. Enek az algoritmusnak nem csak a gerjesztés következő értékére, hanem a két lépés közötti ($\underline{u}[n+0,5]$), melyet könnyedén megoldhatunk a bemenet kétszeres frekvenciájú mintavételezésével, vagy a két egymást követő érték átlagának kiszámításával.