...
\Aref{fig:original_model}. ábrán látható a korábban elékszített modell és az egyes blokkok kapcsolata. Jó kiniduló alapot biztosított, jól megfigyelhető rajta a rendszer működése. Ez a modell bemenetként tekint a DC feszültségre, így az én bemeneti modellemet ezen kívül helyeztem el. A terhelés a bemeneti modellben DC terhelőáram formájában jelentkezik, így ezt a jelet elő kellett még állítani. A rendelkezésre álló félhíd modell csak az AC áramot állítja elő kiementként, így ezt módosítani kellett, hogy a DC áram is előálljon. Ezek után a három félhíd áramát összegezve felhasználhatóvá vált az \emph{I_DC} terhelőáram, bemenetként a DC kört is tartalmazó modellemnek.
...
Az \emph{I_{DC}} terhelőáramot az alábbi egyenletek segítségével határozhatjuk meg:
...
\left{
\begin{array}{rc}
I_{AC} &\mbox{ha}  & PWM_H = 1 | PWM_H = 0 \& I_{AC} < 0 \\
0 & &
\end{array}
}
$$    
